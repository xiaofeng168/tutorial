...
LV_EVENT_CB_DECLARE(print_open_event_cb);  /* 时间回调函数声明 */
...
static void home_open(uint32_t delay) {
    ...
    lv_obj_clean_style_list(cont, LV_CONT_PART_MAIN);  /* 样式重置为默认状态 */
...
/* 点击并加载后的进入界面 退出该界面时定义scan_img = NULL */
static void scan1_open(const char * btn_txt)
{
    ...
    lv_obj_t * back = add_back(back_to_home_event_cb); /* 添加返回箭头 */
    ...
    lv_img_set_src(scan_img, &lv_demo_printer_img_scan_example); /* 小鸟图片 */
    ...
    lv_img_set_src(icon, &lv_demo_printer_icon_bright); /* 明亮图片 */
    ...
    lv_img_set_src(icon, &lv_demo_printer_icon_hue);/* 色彩图片 */
    lv_obj_align_origo(icon, slider, LV_ALIGN_OUT_TOP_MID, 0, -30);
...
/* 点击COPY进入的效果函数实现 */
static lv_obj_t * add_loader(void (*end_cb)(lv_anim_t *)) /* 添加装载 */
{
